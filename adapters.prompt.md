...
2. **用 JS `.click()` 验证 toggle 性**（DOM 事件触发、穿透遮罩）——Playwright 物理点击
   ...
   ≠ 不能 toggle，JS 点击下原按钮可能是天然开关）：连点两次开→关 → `click` 就够；
   仍关不掉 → 走下一步 toggle-panel。
3. `toggle-panel` 关闭通道：优先 `secondClick: {"kind":"mask"}`（点遮罩）——**必须探测验证
   遮罩点击真的会关闭**（remote 弹窗遮罩实测不响应）；不行换
   `{"kind":"click","selector":"弹窗内唯一×按钮"}`。
4. 记录隐藏影响（`hide` 缺省 = 隐藏；弹窗按按钮 rect 定位的按钮设 `hide: false`）。
