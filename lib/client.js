...
function runAdapter(adapter, env) {
	...
	switch (act.kind) {
		...
		case "toggle-panel": {
			...
			if (panel !== null) return actTogglePanel(panel, act.close !== void 0 ? env.find(act.close) : null);
			return false;
		}
		case "open-settings":
	...
	factory: (require) => {
		...
		*/
		...
		function createToolbar() {
			...
			var adapterMenuEl = null;
			var onMenuOutside = function(e) {
				if (adapterMenuEl !== null && !adapterMenuEl.contains(e.target)) closeAdapterMenu();
			};
			...
			function showAdapterMenu(x, y, ad) {
				closeAdapterMenu();
			...
			function removeUserAdapter(ad) {
				...
				fetch("/quick-toolbar/api/adapters").then(function(r) {
					...
					return fetch("/quick-toolbar/api/adapters", {
						...
						if (res !== null && typeof res === "object" && res.ok === true) try {
			...
			function attachAdapterMenu(btn, ad) {
			...
			var fetchUserAdapters = function() {
				fetch("/quick-toolbar/api/adapters").then(function(r) {
					...
					for (var ui = 0; ui < rows.length; ui++) {
						...
						var userBtn = renderButton(user, userKind !== void 0 ? userKind : null);
